...
function AABBx(pos, vec) {

  if(!(this instanceof AABBx)) {
    return new AABBx(pos, vec)
  }
...
function AABB(min, max) {
	...
	this.base = vec3.clone(min);
	this.max = vec3.clone(max);
	this.vec = vec3.create()
	vec3.sub(this.vec, max, min);
	vec3.scale(this.vec, this.vec, 0.5);
	this.mag = vec3.length(this.vec)
}
